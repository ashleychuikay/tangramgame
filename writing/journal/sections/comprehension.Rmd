...
They may be unable to understand or accommodate their partner's description if it does not align with their own way of conceptualizing the tangram, preventing uptake of a pact. In this case, parent-child dyad's communicative success in our reference game may be due to parents' ability to align to their children's points of view, such that children can accurately select the intended referent based on their parent's descriptions.
...
To conceal the identity of the original speaker, the first author and four research assistants produced new audio recordings by reading the Exp.~1 game transcripts in a uniform vocal style. All utterances from the first and fourth (final) round of the reference game were re-recorded.
...
These stimuli were broken into 118 unique 'item sets' containing 10 recordings each, such that each tangram appeared as the target once in each set. The stimuli sets were designed such that all participants encountered the following: 5 utterances generated by a parent and 5 by a child; and 5 utterances from the first round and 5 from the final round. 
...
Child participants provided verbal consent, along with written consent from their parents. Their version of the experiment did not include the initial 'play' button and attention check. While children completed the task independently, an experimenter was nearby to ensure that the audio was working.
...
Participants were not allowed to respond prior to the completion of the audio recording.
...
In Experiment 2, we asked whether children's struggles to form conceptual pacts with others are due to comprehension difficulties. Using a comprehension task, we found that naive adults and children are equally accurate at comprehending referential expressions generated in our reference game. This finding suggests that children are not necessarily poor comprehenders, and that their difficulty forming pacts may lie elsewhere. Further, our results indicate that communicative success in the reference game are not entirely due to parents aligning to their children's idiosyncratic perspectives--even naive listeners readily understood the intended meaning of parents' utterances.

While overall accuracy in our comprehension task was high, we found that referential expressions generated by younger children are less accurately comprehended by adults and children alike. That is, both adults and children are less likely to find the intended referent after hearing a description produced by a 4-year-old child. This points to our second hypothesis: children may struggle to form pacts with peers due to *production* difficulties. We explore this possible explanation in Experiment 3.
