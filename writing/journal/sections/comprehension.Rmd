...
# kid data
stims <- read_csv(here("data/experiment2/random_stims.csv"),
                  show_col_types = FALSE) %>%
...
#write_csv(tidy_kid_data, 'data/experiment2/comprehension_kids_tidy.csv')
```
...
tidy_turk_data %>% 
    unite(audioid, gametrial, gameid) %>% 
    group_by(audioid) %>% 
    tally() %>% 
    mutate(n = n /3) %>% 
    qplot(n, data = .)

tidy_kid_data <- read_csv(here("data/experiment2/comprehension_kids_tidy.csv"))
