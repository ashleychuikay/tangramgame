...
Additionally, we planned to recruited a sample of 200 children (ages 4 to 8) from a school and a museum in the local community, with the relatively large sample size based on the classroom sizes of the local school. However, due to the COVID-19 pandemic, we were forced to terminate data collection early with only `r length(unique(tidy_kid_data$id))` children (ages `r min(tidy_kid_data$age)`-`r max(tidy_kid_data$age)`). 
