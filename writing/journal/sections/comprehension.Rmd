...
To evaluate comprehension, participants were placed in the role of the listener.
On each trial, they were presented with an audio recording of a referring expression from Exp.~1. 
These recordings were produced by the first author reading the transcript in a uniform style, in order to conceal the identity of the original speaker.
After the recording played, they were asked to click on the intended referent and did not receive any feedback.
Each participant proceeded through exactly 10 trials --- a single referring expression for each tangram, paired with its foil from the corresponding trial in Exp.~1.
To detect participants who were not following instructions, we included an attention check which asked people to click 'the one on the left'.
We also did not allow participants to proceed past the consent page without the audio on by asking them to play a recording that said to "type the number 86 into the box."
We measured both accuracy (clicking the correct target) and response time (time interval between the completion of the audio and onset of the response). 
