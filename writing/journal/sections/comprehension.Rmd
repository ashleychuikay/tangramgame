...
group_diff.glmer2.out <- tidy(group_diff.glmer2) %>% filter(term == 'comprehender_groupkid')
group_diff.anova <- anova(group_diff.glmer1, group_diff.glmer2) %>% tidy() %>% filter(term == 'group_diff.glmer2')
```
...
We found no significant difference in overall accuracy across groups, $t=$`r abs(group_diff.glmer2.out$statistic)`, $p =$ `r group_diff.glmer2$p.value`, (adult accuracy $=$ `r comp.means$mean_adult`, 95\% CI $=$ [`r comp.means$ci_lower_adult`, `r comp.means$ci_upper_adult`], child accuracy $=$ `r comp.means$mean_kid`, 95\% CI $=$ `r comp.means$ci_lower_kid`, `r comp.means$ci_upper_kid`).^[While null hypothesis significance testing is unable to provide positive evidence for the null hypothesis, we ran a Bayesian regression using the `brm` package. We used the Savage-Dickey method to compute a Bayes Factor of $9.5$, indicating moderate support in favor of the null hypothesis $b=0$. We used a weakly informative student-$t$ prior on the coefficient with degrees of freedom $\nu = 5$ and scale $s=2.5$, following @gelman2008weakly, but this result was robust to other choices.]
Consistent with this finding, there was no support in a nested comparison for a model with an effect of comprehender group over an intercept-only model, $\chi^2($ `r group_diff.anova$df` $) =$ `r group_diff.anova$statistic`, $p = $ `r group_diff.anova$p.value`.
...
We fit a logistic mixed effects model predicting accuracy as a function of the person who produced the description (parent vs. child), the age of child in the pair (ages 4, 6, 8), and the phase of the game where it was produced (early vs late).
We also include all of their interactions, and random intercepts for each target and participant ID. 
Concerning our first hypothesis, we find that parents' descriptions tend to lead to more accurate $b=XX, z = .., p<0.001$.
Additionally, older kids (and parents of older kids) give better descriptions, . 
But there is no effect of occurrence--descriptions on the second appearance of each item do not appear to contain less information for guessing the target.

Because accuracy was near ceiling, we also consider the measure of response time.
We find that descriptions produced by parents lead to faster guesses.
