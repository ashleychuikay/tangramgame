...
To the extent that there is greater variance in the utterances produced by children, we may expect that lexical priors play a larger role.
...
Children also participated in an additional task designed to measure their understanding of ambiguity in *comprehension* [@markman1977realizing, @beal1990development], as failures to judge the informativity of others' expressions may be implicated in failures to produce appropriately informative utterances of one's own in the close condition (e.g. saying "bear" when both the target and distractor are bears).

In this condition, children were told that they would hear a label produced by another child, and that they should indicate whether this label adequately identified the target object. Three of the trials were far trials in which an unambiguously incorrect label was provided to the child (e.g. "table" to distinguish between a pug and a rabbit), three of the trials were close trials in which an unambiguously correct label was provided (e.g. "polar bear" to distinguish between a polar bear and a grizzly bear), and three were close trials in which an ambiguous label was provided (e.g. "dog" to distinguish between a husky and a dalmatian). For the check condition, children responded verbally ("Yes" or "No") and an experimenter selected the corresponding button on the screen.
...
These hypotheses are not mutually exclusive. Indeed, the corresponding mechanisms -- *pragmatic reasoning* and *lexical priors* -- are both implicated in recent production models [e.g. @murthy2021shades, @hawkins2022partners]. 
...
indiv.measures.lmer <- indiv.measures %>%
  ...
  glmer(far_close_overlap ~ age_group * target_type + utt_length + (1 | target) + (1 + target_type | id),
        family = 'binomial',
        data = .) %>%
  tidy() %>%
  filter(effect == 'fixed') %>%
...
We include the most complex random effects structure that converged, which included random intercepts for both participants and items, and a random slope of target condition at the participant level.

We found a significant interaction between age group and target type, $b=$`r indiv.measures.inter$estimate`, z = `r indiv.measures.inter$statistic`, $p <$ 0.001 (see Fig.~\ref{fig:e3_contextsensitivity}). 
...
Similar results were obtained using 'softer' measures like edit distance, which is the number of edits required to turn one string into the other, $b=$`r soft.inter$estimate`, $z =$ `r soft.inter$statistic`, $p =$ `r soft.inter$p.value` (see Fig.~\ref{fig:Levensthein_appendix}). 
...
One possibility is that children have a less stable prior over possible names for the novel tangram images.
...
We found that children responded as expected to the unambiguous trials. That is, they responded "No" to the incorrect trial ($m=$ `r check_means$mean[3]`), and "Yes" to the unambiguous trial ($m=$ `r check_means$mean[2]`). These findings suggest that children, regardless of age, understood that that goal of the task was to describe a target picture (and not, for instance, to simply say anything). We also found an overall effect of age, where children's accuracy on the check trials increased with age ($p=$ `r age_model$p_value[2]`). Further, we found an effect of trial type, such that children were more likely to be correct on unambiguous ($p=$ `r age_model$p_value[3]`) and incorrect trials ($p=$ `r age_model$p_value[4]`) than on ambiguous trials ($m =$ `r check_means$mean[1]`).
...
kid_ambiguous_model <- kid_ambiguous_model_data %>%
  lmer(empirical_stat ~ target_type*correct + (1|id) + (1|age), data = .) %>%
  tidy() %>%
  filter(effect == "fixed")
...
We fit a mixed-effects model predicting children's context sensitivity, as measured by 'exact match' overlap, from their performance on the ambiguous trials and target type (familiar vs. tangram), with random effects of participant and age. We find a significant interaction between accuracy on the ambiguous trials and target type, such that children who are more accurate on the ambiguous check trials are more likely to modulate their descriptions based on context for familiar items ($b=$ `r kid_ambiguous_model%>%filter(term == "correct")$estimate`,$t$(`r kid_ambiguous_model%>%filter(term == "correct")$df`)$=$`r kid_ambiguous_model%>%filter(term == "correct")$statistic`, $p=$`r kid_ambiguous_model%>%filter(term == "correct")$p.value`). In other words, children who are more sensitive to referential ambiguity show less exact matches of their descriptions of familiar items between close and far contexts. However, we do not find the same effect on tangram trials. 
