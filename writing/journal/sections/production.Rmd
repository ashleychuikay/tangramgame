...
d.prod.adult <- read_csv(here("data/production/anonymizedDataFromMongo.csv"),
...
d.prod.kid <- list.files(here("data/production/kid/"), "*.csv",
                          full.names = TRUE) %>%
...
```{r e2-child-data}
n_kid_stims <- kid_production_data %>%
  distinct(target_type, competitor_type, target, foil) 

kid_fam_stims <- n_kid_stims %>%
  filter(target_type == "familiar") %>%
  distinct(target, foil) %>%
  unlist() %>%
  unique()
...
We recruited `r length(complete_games.adult)` adult participants from Mechanical Turk. All participants gave informed consent prior to the start of the study and were compensated XX dollars. 

We also recruited XX children to participate in the study. DEMOGRAPHICS XX. The study was conducted online over Zoom.
