...
```{r e3-read-adult-data, eval = FALSE}
NUM_TRIALS <- 48
...
contextsensitivity.means %>%
  ggplot(aes(x = age_group, y = empirical_stat, fill = target_type)) +
    ...
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0, 
                  position = dodge) +
    labs(y = '% overlap (exact matches)', x = 'Participant age group') +
...
We found that children responded as expected to the unambiguous trials. That is, they responded "No" to the incorrect trial ($m=$ `r check_means%>%filter(check_type == "incorrect")$mean`), and "Yes" to the unambiguous trial ($m=$ `r check_means%>%filter(check_type == "unambiguous")$mean`). These findings suggest that children, regardless of age, understood that that goal of the task was to describe a target picture (and not, for instance, to simply say anything). We also found an overall effect of age, where children's accuracy on the check trials increased with age ($b=$ `r age_model%>%filter(term == "age")$estimate`, $t$(`r age_model%>%filter(term == "age")$df)`$=$ `r age_model%>%filter(term == "age")$statistic`, $p=$ `r age_model%>%filter(term == "age")$p.value`). Further, we found an effect of trial type, such that children were more likely to be correct on unambiguous ($b=$ `r age_model%>%filter(term == "check_typeunambiguous")$estimate`, $t$(`r age_model%>%filter(term == "check_typeunambiguous")$df`)$=$ `r age_model%>%filter(term == "check_typeunambiguous")$statistic`, $p=$ `r age_model%>%filter(term == "check_typeunambiguous")$p.value`) and incorrect trials ($b=$ `r age_model%>%filter(term == "check_typewrong")$estimate`, $t$(`r age_model%>%filter(term == "check_typewrong")$df`)$=$ `r age_model%>%filter(term == "check_typewrong")$statistic`, $p=$ `r age_model%>%filter(term == "check_typewrong")$p.value`) than on ambiguous trials ($m =$ `r check_means%>%filter(check_type == "ambiguous")$mean`).
...
```{r e3-check, out.height = "300px", fig.align = "center", set.cap.width=T, fig.cap = "\\label{fig:e3_check} Children's performance on ambiguous manipulation trials predicted their context sensitivity for familiar objects, but not for tangrams."}
...
We fit a mixed-effects model predicting children's context sensitivity, as measured by 'exact match' overlap, from their performance on the ambiguous trials and target type (familiar vs. tangram), with random effects of participant and age. We find a significant interaction between accuracy on the ambiguous trials and target type, such that children who are more accurate on the ambiguous check trials are more likely to modulate their descriptions based on context for familiar items ($b=$ `r kid_ambiguous_model%>%filter(term == "target_typefamiliar:correct")$estimate`,$t$(`r kid_ambiguous_model%>%filter(term == "target_typefamiliar:correct")$df`)$=$`r kid_ambiguous_model%>%filter(term == "target_typefamiliar:correct")$statistic`, $p=$`r kid_ambiguous_model%>%filter(term == "target_typefamiliar:correct")$p.value`). In other words, children who are more sensitive to referential ambiguity show less exact matches of their descriptions of familiar items between close and far contexts (Fig.\ref{fig:e3_check}). 
