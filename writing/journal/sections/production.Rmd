...
```{r e2-child-data}
kid_production_files <- list.files(here("data/production/kid/"), "*.csv",
                                   full.names = TRUE)

kid_production_data <- map_dfr(kid_production_files, read_csv, 
                               show_col_types = FALSE)

n_kid_stims <- kid_production_data %>%
  distinct(target_type, competitor_type, target, foil) 

kid_fam_stims <- n_stims %>%
  filter(target_type == "familiar") %>%
  distinct(target, foil) %>%
  unlist() %>%
  unique()
...
We recruited `r length(complete_games)` adult participants from Mechanical Turk. All participants gave informed consent prior to the start of the study and were compensated XX dollars. 
