...
```{r}
d.prod.raw <- read_csv(here("data/production/pilot_data.csv"))
...
d.prod <- d.prod.raw %>%
  ...
  filter(anonymized_id %in% complete_games) %>%
  filter(rt < 20000) %>%
  mutate(utterance = str_to_lower(utterance),
         utterance = str_trim(utterance),
         utterance = str_remove_all(utterance, pattern = ' '),
         utt_length = str_length(utterance),
         clean_utt = lemmatize_words(utterance))
...
```{r}
ggplot(d.prod.clean, aes(x = target_type, y = log(rt), color = competitor_type)) +
  geom_boxplot() +
  theme(legend.position = 'top')
```
...
indiv_measures <- d.prod.clean %>%
  pivot_longer(cols = c(utterance, clean_utt), 
               names_to = "type", values_to = "utterance") %>%
  select(anonymized_id, target, target_type, competitor_type, type, utterance) %>%
  group_by(anonymized_id, target_type, target, type) %>%
...
d.raw.labels <- read_csv(here('./data/production_tangram_only/anonymizedDataFromMongo.csv')) 
...
d <- d.raw.labels %>%
  ...
  mutate(lemmatized_utt = str_replace_all(lemmatized_utt, regex("\\W+"), ""))
```
...
```{r get-pair-kl}
get_kl_data <- function(df, tangram1, tangram2) {
  df %>%
...
pairs <- d %>% distinct(target) %>%
  ...
  nest() %>%
  mutate(kl = map(data, ~get_kl_data(kl_input_data, .x$tangram1, .x$tangram2) %>%
                    kl_drop(., target_n, comparison_n))) %>%
  select(-data) %>%
  unnest(cols = c(kl))

symmetric_pairs <- pairs %>%
  mutate(tmp = target_id, 
...
pilot_items <- d.prod.clean %>% 
  ...
  left_join(symmetric_pairs, by = c("target_id", "comparison_id"))
...
```{r plot kl} 
ggplot(pairs, aes(x = target_id, y = comparison_id, fill = kl)) + 
  geom_tile()
```
