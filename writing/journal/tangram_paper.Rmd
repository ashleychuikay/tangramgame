...
```{r}
d.prod.raw <- read_csv('../../data/production/pilot_data.csv')
...
d.prod.clean <- read_csv('../../data/production/pilot_data_clean.csv') %>%
  filter(utt_length > 1) %>%
  mutuate(clean_utt = lemmatize_words(clean_utt))
...
d.prod %>%
  select(anonymized_id, target, target_type, competitor_type, utterance) %>%
  group_by(anonymized_id, target_type, target) %>%
