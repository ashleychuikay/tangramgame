...
```{r}
d.prod <- read_csv('../../data/production/pilot_data.csv') %>%
...
d.prod %>%
  ...
  # tidyboot_mean(value) %>%
  ggplot(aes(x = target_type, y = value, fill = competitor_type)) +
    geom_bar(stat = 'identity', position=dodge) +
    facet_grid(target~measure, scales='free')

ggsave('variability_measures.pdf', width = 10, height = 10, unit = 'in')
```

# TODO: Measuring similarity within participant
