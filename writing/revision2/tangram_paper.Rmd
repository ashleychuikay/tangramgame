...
affiliation:
  - id            : "1"
    institution   : "Department of Psychology\\comma\\hspace{.25em} Harvard University"
  - id            : "2"
    institution   : "Department of Psychology\\comma\\hspace{.25em} Carnegie Mellon University"
  - id            : "3"
    institution   : "Department of Psychology\\comma\\hspace{.25em} University of Wisconsin-Madison"
...
authornote: |
  All data and code for these analyses are available at https://osf.io/vkug8/?view_only=b68061f8daae48cdace5c3dc53969405. Studies 2-3 were preregistered at https://osf.io/vkug8/registrations; the preregistration for Study 1 is unavailable due to an archiving error on OSF. Videos of experiment sessions are available on Databrary.
...
It is possible, however, that parents may *spontaneously* scaffold referential pacts with their children in a similar way.
Parents are not only more linguistically capable than their children, but also have fine-grained representations about their children's knowledge and conversational abilities [@leung2021]. 
...
Our paradigm differs in important ways from those used in prior work and it is possible that pairs of children would, hypothetically, succeed at forming conversational pacts without experimenter intervention in our variant. 
For example, our task used contexts containing only two tangram images per trial, whereas classic studies used contexts of six or twelve. 
This choice could have reduced the cognitive load in our variant.
On the other hand, participants were instructed to communicate verbally without the ability to use non-verbal modalities like gesture that they could rely upon in other paradigms [@holler2022visual; @lister2021cross; @bohn2019young], making the task more challenging. 
There are other reasons to believe that pairs of children would still struggle in our variant, including the production failures we independently observed in Experiment 3 and the observation than younger children relied more heavily on parent-generated descriptions in Experiment 1. 
...
Given that the labels used by younger children tend to originate with their parents, it is tempting to conclude that adaptation for these pairs is entirely one-sided. However, the resulting pacts may have nonetheless been shaped by children. In particular, we suggest that even though young children may not be able to *supply* good initial labels, they may serve as "simplifiers" or "filters" that constrain the resulting pact, since it must pass through their comprehension and production system [@hudson-kam2005].
We tested this hypothesis by asking about the age of acquisition (AoA) of the final pacts developed in each age group [@kuperman2012]. 
To estimate the average age of acquisition of referential expressions as a whole, we simply averaged the AoAs of all the words they contained^[We first removed any words in a list of `r length(tm::stopwords())` stop words like "at" and "me" from the \texttt{tm} package [@feinerer2008]]. 
As children get older, we observed that both their own expressions and their parents' concentrate on higher AoA phrases over repetitions of the same tangram (like adults) (Figure \ref{fig:aoa_appendix}). 
...
Here, we examine which features of the descriptions may affect downstream comprehension.
...
```{r appendix-comprehension, out.height = "450px", fig.align = "center", set.cap.width=T, fig.cap = "\\label{fig:comprehension_age_appendix} Accuracy of comprehender by age. Dashed line represents adult comprehension"}

comp.means.age <- bind_rows(tidy_turk_data, tidy_kid_data)  %>%
  filter(measure == "correct") %>%
  mutate(p_age = ifelse(comprehender_group == 'adult', 'adult', p_age)) %>%
  filter(p_age %in% c(4,5,6,7,8,'adult')) %>%
  group_by(p_age) %>%
  tidyboot_mean(value) %>%
  select(-n) 
...
comp.means.age %>%
  filter(p_age != 'adult') %>%
  ggplot(aes(x = p_age, y = empirical_stat)) +
    geom_point() +
    geom_line() +
    geom_hline(aes(yintercept = empirical_stat), data = comp.means.age %>% filter(p_age == 'adult'), linetype = 'dashed') +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0) +
    labs(y = 'comprehension accuracy', x = 'age') +
    ylim(0,1) +
    theme_few()
```

Finally, we consider whether there was an effect of comprehender age on accuracy.
We constructed a logistic mixed-effects model with a fixed effect for comprehender age (numeric) and random intercepts for each participant. 
We observed a numerically small but significant effect of age on comprehension accuracy, $b=0.29, z = 3.12, p = 0.002$, which appears to be primarily driven by the small group of four-year-olds with poorer accuracy (mean accuracy of 62\%, CI: [47\%, 77\%], compared to approximately 87\% for older children).
Thus, while comprehension accuracy in our overall sample of children is not significantly different from adult comprehension, it is possible that this is a result of older children being overrepresented in our sample. 
...
## Appendix E: Specificity of referring expressions for familiar objects

```{r appendix-specificity, out.height = "450px", fig.align = "center", set.cap.width=T, fig.cap = "\\label{fig:specificity_appendix} Proportion of descriptions which are appropriately informative by age."}
#annotated context sensitivity
contextsensitivity.annotated <- d.prod.clean.annotated %>%
  filter(age_group == "kid") %>%
  filter(target_type == "familiar") %>%
  filter(competitor_type != "check") %>%
  mutate(informativity = ifelse(informativity > 1, 1, informativity)) %>%
  group_by(age, competitor_type) %>%
  tidyboot_mean(informativity, na.rm = T) 

contextsensitivity.annotated.model <- d.prod.clean.annotated %>%
  filter(age_group == "kid") %>%
  filter(target_type == "familiar") %>%
  filter(competitor_type != "check") %>%
  filter(competitor_type == 'close') %>%
  mutate(informativity = ifelse(informativity > 1, 1, informativity)) %>%
  glmer(informativity ~ scale(age) + (1 | id) + (1 + scale(age) | target),
        family = 'binomial',
        data = .) 

contextsensitivity.annotated %>%
  ggplot(aes(x = age, y = empirical_stat, color = competitor_type)) +
    geom_point() +
    geom_line() +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0) +
    labs(y = 'informativity', x = 'age') +
    theme_few()
```

Our analysis in Experiment 3 established that younger children were less likely to produce distinct referring expressions across close and far contexts.
However, this analysis leaves open the possibility that they were producing the same *overinformative* utterance, e.g. calling the target a 'pug' in both contexts even when 'dog' would have been sufficient in the far context.
To confirm that the decreased context-specificity displayed by children actually reflects pragmatic underspecificity, we hand-annotated each utterance in the familiar condition for their degree of informativity. 
That is, we gave a 0 to underinformative utterances which are true of both objects in context (e.g. saying ‘dog’ in a close context with two dogs) and we gave a 1 to appropriately informative utterances which are true of only the target (e.g. saying ‘pug’ in a close context where the distractor is not a pug). 

We found that children indeed frequently produced less specific utterances than required, with only 68\% of 'close' trials being appropriately informative, relative to 99\% of 'far' trials.
These findings suggest that children's descriptions are not only context-insensitive, they are less informative than pragmatically required for the 'close' context.
Intriguingly, we also found a strong age effect (Figure \ref{fig:specificity_appendix}). 
Only 16\% of 4-year-olds produced appropriately informative utterances in the 'close' condition, compared to 93\% of 8-year-olds, $b = 1.97, z = 4.39, p < 0.001$ in a logistic mixed effects regression model with random intercepts for participant ID and target ID. 
Thus, children produce increasingly informative utterances over time. 
...
```{r appendix-full-accuracy-model}
lmer.accuracy %>%
  filter(effect == 'fixed') %>%
  select(-effect, -group) %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:accuracy}Output of full Exp. 1 accuracy model: \\texttt{accuracy $\\sim$
    age + director + rep\\_num + (rep\\_num | subid) + (1 | target)}"
  )
```

```{r full-exchanges-model}
lmer.exchanges %>%
  filter(effect == 'fixed') %>%
  select(-effect, -group) %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:exchanges}Output of full Exp. 1 exchanges model: \\texttt{exchanges $\\sim$ age +
    rep\\_num +  (1 + rep\\_num | subid) + (1 + rep\\_num | target)}"
  )
```

```{r full-reduction-model}
lmer.reduction.controlled %>%
  filter(effect == 'fixed') %>%
  select(-effect, -group) %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:reduction}Output of full Exp. 1 reduction model: \\texttt{num\\_words $\\sim$
    person * age * rep\\_num + (1 + person * rep\\_num | subid) + (1 + rep\\_num | target))}"
  )
```
...
match.model %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:match}Output of full Exp. 1 pact origin model: \\texttt{introduced
    $\\sim$ age * director + (1 | target) +  (1 | subid)}"
  )
```
...
bind_rows(tidy_turk_data, tidy_kid_data)  %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:comprehension}Output of full Exp. 2 comprehension model: \\texttt{accuracy $\\sim$ comprehender\\_group + age + person + occurrence + (1 + comprehender\\_group + person | gameid)}"
  )
```
...
brms_out %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:comprehension2}Output of Exp. 2 Bayesian model with stimulus-level random effects: \\texttt{accuracy $\\sim$ comprehender\\_group + (1 + comprehender\\_group | audioid)}"
  )
```

```{r appendix-full-production-model, warning = FALSE, message = FALSE}
indiv.measures.lmer %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:production}Output of full Exp. 3 context sensitivity model: \\texttt{overlap $\\sim$
    age\\_group * target\\_type + scale(utt\\_length) + (1 | id)}"
  )
```

```{r appendix-full-production-model2, warning = FALSE, message= FALSE}
indiv.measures.softmeasure %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:production2}Output of Exp.3 context sensitivity model using edit distance:
    \\texttt{distance $\\sim$ age\\_group * target\\_type + (1 | target) + (1 + target\\_type | id)}"
  )
```

```{r appendix-full-production-model3, warning = FALSE, message= FALSE}
var.out %>%
  select(-group, -effect) %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:production3}Output of Exp.3 model for unique labels:
    \\texttt{unique labels $\\sim$ target\\_type * age\\_group + (1 | target)}"
  )
```

```{r appendix-full-production-model4, warning = FALSE, message= FALSE}
var.out.entropy %>%
  select(-group, -effect) %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:production4}Output of Exp.3 entropy model:
    \\texttt{entropy $\\sim$ target\\_type * age\\_group + (1 | target)}"
  )
```