...
lmer.accuracy %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:accuracy} Output of full Exp. 1 accuracy model: \\texttt{accuracy $\\sim$ comprehender\\_group + age + person + occurrence + (1 + comprehender\\_group + person | gameid)}"
  )
```
...
lmer.exchanges %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:exchanges} Output of full Exp. 1 exchanges model: \\texttt{exchanges $\\sim$ age + occurrence + (1 + occurence | subid) + (1 + occurrence | target)}"
  )
```
...
lmer.reduction.controlled %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:reduction} Output of full Exp. 1 reduction model: \\texttt{exchanges $\\sim$ comprehender\\_group + age + person + occurrence + (1 + comprehender\\_group + person | gameid)}"
  )
```
...
match.model %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:match} Output of full Exp. 1 pact origin model: \\texttt{exchanges $\\sim$ comprehender\\_group + age + person + occurrence + (1 + comprehender\\_group + person | gameid)}"
  )
```
...
```{r appendix-full-production-model}
indiv.measures.lmer %>%
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:production}Output of full context sensitivity model: \\\texttt{}"
  )
```
