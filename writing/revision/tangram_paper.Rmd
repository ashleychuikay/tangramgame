...
lmer.accuracy %>%
  ...
  kable(
    ...
    age + director + rep_num + (rep_num | subid) + (1 | target)}"
  )
```
...
lmer.exchanges %>%
  ...
  kable(
    ...
    rep_num +  (1 + rep_num | subid) + (1 + rep_num | target)}"
  )
```
...
lmer.reduction.controlled %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:reduction}Output of full Exp. 1 reduction model: \\texttt{number of words $\\sim$
    person * age * rep_num + (1 + person * rep_num | subid) + (1 + rep_num | target))}"
  )
```
...
match.model %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    caption = "\\label{tab:match}Output of full Exp. 1 pact origin model: \\texttt{words introduced by partner
