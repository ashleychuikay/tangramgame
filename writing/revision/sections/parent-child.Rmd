...
Twelve solid black images of tangrams were normed for pairwise similarity by an independent group of 60 participants on Amazon Mechanical Turk. Each of these participants made 22 pairwise similarity judgments on a scale from 1-100. Based on these similarity ratings ($M$ = 42.3, $SD$ = 26.1), the ten tangrams with the highest overall pairwise dissimilarity were selected for use as stimuli (Figure \ref{fig:ipads}A).
To ensure that the game would not be too difficult, we designed contexts such that the foil was never too similar to the target.
To do so, we rank-ordered all tangram pairs from least to most similar and chose foils to minimize similarity while ensuring that each image appeared as a foil 4 times. 

### Design and procedure

Pairs of participants were brought into the lab to play a cooperative director-matcher game. They were seated in front of iPads at opposite ends of a table, with a divider preventing them from seeing the other's screen. This divider did not fully occlude participants; they could still see their partner's face. Participants were told that they would take turns playing *director* and *matcher* roles. On each trial, a context of exactly two tangrams appeared on their screens. One of these tangrams was the target, and the other was the foil. The director's task was to describe the *target* image, privately indicated by a blue border, and the matcher's task was to select one of the two images on their screen based on the director's description (Figure \ref{fig:ipads}B). Before beginning the experiment, participants played six practice trials with images of common fruits and vegetables. 
...
We quantified dialogue exchanges by counting the total number of distinct turns of continuous speech on each trial and constructed a (Poisson) mixed-effects model predicting the (continuous) number of exchanges with the same effect structure reported in the previous section. 
...
```{r matches}
# This csv is produced by /analysis/content_analyses.ipynb
...
Additionally, we found a weak but significant interaction between director and age, indicating that this asymmetry was smaller for older children ($\beta=$ `r match.interaction_effect$estimate`, $t=$ `r match.interaction_effect$statistic`, $p=$ `r papaja::printp(match.interaction_effect$p.value)`). 
Thus, parents---especially parents of younger children---appear to be the source of the labels that persist in successful pacts.

## Summary and discussion

In Experiment 1, we adapted the classic tangram director-matcher paradigm developed by @clark1986 to examine the formation of conversational pacts among parent-child dyads. 
We found that even 4-year-old children and their parents can successfully coordinate on pacts, but for these youngest children, parents provide multiple sources of spontaneous support and scaffolding, including more back-and-forth exchanges initiating clarification as well as simplified referring expressions (see Appendix B). Of note is that our task did not offer contingent feedback for participants: the same sounds played regardless of whether the selection was correct. The original intent was for feedback to be consistent and neutral, though we acknowledge that contingent feedback could be helpful for pact formation. However, the extent to which parent-child pairs were successful in this task suggests that children's performance can be facilitated by parental scaffolding, even in the absence of other factors such as explicit feedback.
Given such scaffolding, 4-year-old children readily adopted informative labels introduced by their parents and reduced the length of their referring expressions in similar ways to dyads with older children and adults. 
We are unable to directly compare the performance of our parent-child dyads against earlier studies of peer-to-peer interactions.
However, these findings raise a number of questions about how, exactly, parental scaffolding may help. 
