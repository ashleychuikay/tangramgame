...
```{r ipads, fig.env = "figure", fig.pos = "tb", fig.width=3.5, fig.height=2, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Example of iPad screens for the director (left) and matcher (right) during the game."}
...
Children (ages 4, 6, and 8) and their parents were recruited from a database of families in the local community to achieve a planned sample of 60 parent-child pairs. A total of 75 children and their parents participated, but data from 12 pairs were dropped due to experimental error or failure to complete the study, leaving a final sample of 63 pairs. For comparison, a group of adult participants were also recruited from a Psychology Department subject pool to achieve a planned sample of 20 adult-adult pairs. At the time of analysis, only 15 pairs have completed the experiment and had their data transcribed.

### Stimuli 

Twelve solid black images of tangrams were selected from a database of Public Domain images. These tangram images were normed on Amazon Mechanical Turk (mTurk) for pairwise similarity in prior study. Ten tangrams were selected based on these similarity judgments in order to construct trials in which tangrams were maximally dissimilar.

### Design and procedure

Pairs of participants played a cooperative game with iPads adpted from matcher-director games used in previous studies [e.g. @krauss1969]. They sat at a table with a divider in the middle, preventing them from looking at each other's screens during the game.

Participants were told that they would take turns being the director and matcher. The director would describe the image inside the blue square, and the matcher should select an image based on the director's description. After instruction, the practice and experimental trials began. 

On each experimental trial, two black tangrams appeared on the iPad screens. Identical images appeared on each participant's screen, but their positions were randomized. On the director's screen, one image appeared inside a blue square, while the images on the matcher's screen simply appeared on a white background (Figure \ref{fig:ipads}). When the matcher selected an image, it would become colorful and a pleasant sound played (whether the selection correct or not). After each trial, the matcher and director switched roles. The two practice trials were identical except that the images were of known fruits and vegetables in order to orient participants to the game. 

There game consisted of four blocks of ten trials each. Each tangram appeared as the target once during each block, and so was the target four total times. Trials within blocks were randomized. The ten tangrams were randomly divided into two groups of five. Each player began the first round as the director for one group of five, and the other play the other group of five. Each player swapped roles for each tangram each round, being the director twice and the matcher twice. In each round, the target tangram appeared with different foils. To ensure that the game would not be too difficult for young children, trials were constructed such that the most similar tangrams (based on mTurk worker judgments) did not appear together.
...
```{r reduction, cache=T, fig.env = "figure*", fig.pos = "tb", fig.width=7, fig.height=3, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "\\label{fig:reduction}Total number of words in referring expressions produced by children and parents over the course of interaction."}
...
d.reduction.means %>%
  ...
  xlab("repetition #") +
  scale_color_manual(values = c(SCALE, "orange"))
```
...
```{r origins, cache=T, fig.env = "figure*", fig.pos = "tb", fig.width=7, fig.height=4, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "\\label{fig:origins}(A) Probability of words used on final round first occuring with child or parent. (B) Complexity of language used by different age groups. Error bars are 95\\% CI."}
...
\todo[inline]{TODO: Shorten AoA paragraph + finish sections}
