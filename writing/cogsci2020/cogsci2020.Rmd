...
```{r accuracy, cache=T, fig.env = "figure", fig.pos = "h", fig.width=4, fig.height=2, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Accuracy \\todo[inline]{TODO: We should combine these basic measures into a single 2-col figure.}"}
...
We operationalize efficiency as the total number of words produced by the *director* on each trial. 
Because participants in a pair alternated roles, each participant served as the director twice for each tangram. 
This structure allows us to examine how each participant changed their language when they were the director (Fig. \ref{fig:reduction}). 

Using a mixed-effects model, we predicted the (log) number of words on each trial, including fixed effects of child age group, repetition block, and speaker identity (parent vs. child) as well as all of their interactions.
...
All variables were centered to allow interpretation of particular lower-order terms as effects at the average level of the other terms. 
...
Because the total number of words produced on a trial is correlated with the number of dialogue exchanges examined above ($r=$`r word_exchange_cor`), we next conducted a supplementary analysis to control for dialogue.
Specifically, we divided the total number of words by the number of exchanges and used this normalized measure as the dependent variable...
