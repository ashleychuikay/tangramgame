...
length_lm <- lmer(log(length ) ~ age * trial + correct + 
                   (correct + trial| subid) + (1| target), 
...
turns_lm <- lmer(log(nturns) ~ age * trial + correct + 
                   (correct + trial| subid) + (1| target), 
                 control = lmerControl(optimizer = "bobyqa"),
                 data = model_data)
...
model_indiv_data <- tidy_data %>%
  ...
  summarise(length = sum(length, na.rm = T),
            nturns = n()) %>%
  group_by(age, person, subid, target) %>%
  arrange(target, .by_group = T) %>%
  mutate(trial = 1:n()) 


indiv_length_lm <- lmer(log(length) ~ age * trial + age * person + 
                          person * trial + correct + 
                 (person + trial + correct |subid) + (1| target),
                 control = lmerControl(optimizer = "bobyqa"),
...
indiv_turns_lm <-  lmer(log(nturns) ~ age * trial + person + correct + 
                 (person + trial |subid) + (1| target),
                 control = lmerControl(optimizer = "bobyqa"),
                 data = model_indiv_data)
