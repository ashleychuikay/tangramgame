...
```{r}
d.exchanges.raw <- read_csv('../data/deidentified/exchanges.csv') %>%
  filter(experiment == 'adult-child') %>%
  mutate(age = ifelse(is.na(age), 'adult', age)) %>%
  group_by(subid, trial, age, person, role, rep_num) %>%
  summarize(num_exchanges = n()) %>%
  unite(person_role, person, role) %>%
  pivot_wider(names_from = person_role, values_from = num_exchanges, values_fill = 0) %>%
  pivot_longer(cols = c(parent_director, parent_matcher, child_director, child_matcher),
               names_to = 'person_role',
               values_to = 'num_exchanges')

d.exchanges.boot <- d.exchanges.raw %>%
  group_by(age, person, role) %>%
  tidyboot_mean(num_exchanges) %>%
  ungroup()
```
