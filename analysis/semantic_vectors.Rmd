...
M_mat = read_csv('../data/deidentified/meta_tangrams_embeddings.csv', 
  ...
  mutate(feature_ind = row_number(),
         rep_num = floor((trial -1) / 10),
         rep_num = ifelse(rep_num == 3, 1, 0)) %>%
...
true_lmer.within.toplot %>%
  ggplot(aes(x = age, y = empirical_stat)) +
    ...
    ylim(0.4, 1) +
    labs(x = 'age', y = 'cosine similarity b/w first & final rep') +
