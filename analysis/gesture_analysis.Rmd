...
authors: "Ashley Leung, Karina Kling, and Dan Yurovsky"
...
kid_visible_gestures <- kid_data %>%
  group_by(age) %>%
  tidyboot_mean(cbarrier, na.rm = T)

kid_speech <- kid_data %>%
  group_by(age) %>%
  tidyboot_mean(cspeech, na.rm = T)

# parent's gestures

parent_gestures <- parent_data %>%
  group_by(age) %>%
  tidyboot_mean(pgesture) %>%
  mutate(age = as.factor(age))
...
parent_visible_gestures <- parent_data %>%
  group_by(age) %>%
  tidyboot_mean(pbarrier, na.rm = T)

parent_speech <- parent_data %>%
  group_by(age) %>%
  tidyboot_mean(pspeech, na.rm = T)
...
kid_model <- kid_data %>%
  group_by(age, subid) %>%
  glmer(cgesture ~ age + (1|subid), family = "binomial", data = .) %>%
  tidy() %>%
  filter(effect == "fixed")
...
parent_model <- parent_data %>%
  group_by(age, subid) %>%
  glmer(pgesture ~ age + (1|subid), family = "binomial", data = .) %>%
  tidy() %>%
  filter(effect == "fixed")
